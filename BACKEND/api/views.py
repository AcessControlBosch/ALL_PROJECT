...
class CourseAPI(APIView):
    ...
    def get(self, request, pk=''):
        ...
        if pk == '':
            ...
            serializer = CourseTable(courseResult, many=True)
            return Response(serializer.data)

        else:
            courseResult = Course.objects.get(id=pk)
            serializer = CourseTable(courseResult)
            return Response(serializer.data)

    def post(self, request):

        serializer = CourseTable(data=request.data, many=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()        
        return Response({"msg": "Inserido com sucesso"})
    ...
    def put(self, request, pk=''):
        ...
        courseResult = Course.objects.get(id=pk)
        serializer = CourseTable(courseResult, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
